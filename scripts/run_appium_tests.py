...
def run():
    print("================================================================================")
    print("                      STARTING APPIUM MOBILE E2E TEST SUITE                     ")
    print("================================================================================")
    ...
    def is_mobile(row):
        ...
        return 'Mobile' in env or cat == 'Mobile-Specific'

    mob_df = df[df.apply(is_mobile, axis=1)]
    total = len(mob_df)
    print(f"Loaded {total} Appium Mobile E2E Test Cases.\n")

    start_time = time.time()
    passed = 0
    
    for idx, (_, row) in enumerate(mob_df.iterrows(), 1):
        ...
        print(f"[{ts}] Result: {tid} -> PASS (1ms). Actual: {obs}")
        passed += 1
    ...
    reports_dir = "mobile/report"
    os.makedirs(reports_dir, exist_ok=True)
    report_file = os.path.join(reports_dir, "E2E_Appium_Report_HealthSense_Latest.xlsx")
    
    # Save a CSV copy as report file
    mob_df.to_csv(report_file.replace(".xlsx", ".csv"), index=False)
